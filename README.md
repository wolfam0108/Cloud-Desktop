...
Нет готовых решений по настройке и созданию полноценного рабочего стола (Cloud Desktop) внутри контейнера, в моём случае LXC используемый Proxmox, с ускорением, без использования DRM master. Исследование показало то что есть некий заложенный функционал в KWin и Sunshine. Но все это ведёт к тому что задача не выполняется.
...
### Почему не `kwin_wayland --virtual` напрямую?

Логичный вопрос: у KWin есть режим `kwin_wayland --virtual` — headless Wayland-композитор без физического монитора. Казалось бы, достаточно запустить его + Sunshine и получить cloud desktop. Этот путь был пройден первым, и он **не работает** без серьёзных модификаций:

| Проблема | Описание |
|---|---|
| **Захват экрана** | Sunshine использует `xdg-desktop-portal` → PipeWire. Portal требует GUI-диалог подтверждения — невозможно в headless |
| **Патч xdg-desktop-portal** | Sunshine отправляет `persist_mode=2` + `restore_token`, portal отвечает fatal error: *"Remote desktop sessions cannot persist"*. Пришлось патчить исходники |
| **Патч xdg-desktop-portal-kde** | `SelectDevices` возвращает ошибку при отсутствии устройств, `Start` показывает GUI-диалог. Пришлось добавить `XDG_PORTAL_HEADLESS_AUTO_ACCEPT=1` |
| **Ввод: нет libinput** | `--virtual` режим **не использует libinput** — не видит `/dev/input/eventX`. Sunshine создаёт uinput-устройства, но KWin их игнорирует |
| **Python input bridge** | Пришлось написать мост: evdev → D-Bus RemoteDesktop portal → KWin. Нестабильно, колесо мыши глючит, задержка непредсказуема |
| **Нет GPU-оптимизации** | Путь PipeWire → portal → capture не гарантирует zero-copy. Нет контроля над GBM буферами и Vulkan pipeline |

Итог: 2 патченных системных пакета + Python-скрипт для ввода + нестабильный захват без GPU-оптимизации. При обновлении системы (`dnf upgrade`) патчи перезаписываются.

Gamescope решает **все эти проблемы архитектурно**: screencopy вместо портала, libinput напрямую, zero-copy GBM pipeline, никаких патчей сторонних пакетов.
...
- **Remote Play** — Steam видит KDE desktop и позволяет стримить через свой собственный протокол, можно добавить steam в автозапуск, и запускать игры без Sunshine.
